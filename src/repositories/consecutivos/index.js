...
const Model = require('../../models/consecutivos');
  ...
  listar: async (objData) => {
    try {
      ...
      if (response.length > 0) {  
        await Model.findOneAndUpdate(query, {Valor: response[0].Valor + 1}, { new: true })
        response[0].Valor = response[0].Valor + 1;
      }
      //set values
